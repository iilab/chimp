...
var GridManager = require('./grid-manager.js');
...
function SessionManagerFactory(options) {
	  ...
	  if (options.host && (options.host.indexOf('browserstack') > -1 || options.host.indexOf('saucelabs') > -1 || options.host.indexOf('testingbot') > -1 || options.host.indexOf('hub') > -1)) {

		  if ((!options.user || !options.key) && !(options.host.indexOf('hub') > -1)) {
    ...
    } else if (options.host.indexOf('hub') > -1) {
